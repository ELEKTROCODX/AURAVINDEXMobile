...
fun ListsScreen(
    ...
    LaunchedEffect(Unit) {
        bookListViewModel.loadUserLists(
            token = settings.value[SettingKey.TOKEN.keySetting].orEmpty(),
            userId = settings.value[SettingKey.ID.keySetting].orEmpty()
        )
    }
                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    if (showForm) {
                        ListForm(
                            ...
                            bookListViewModel = bookListViewModel,
                            user = settings.value[SettingKey.ID.keySetting].orEmpty(),
                            token = settings.value[SettingKey.TOKEN.keySetting].orEmpty(),
                            context = context
                        )
                    }
                    ...
                    val filteredLists = userLists
                        ?.filter { it.owner._id == settings.value[SettingKey.ID.keySetting] }
                        ?.sortedBy { it.title != "Favorites" }
                        ?: emptyList()

                    BookListCardList(
                        bookLists = filteredLists,
                        navController = navController,
                        bookListViewModel = bookListViewModel,
                        token = settings.value[SettingKey.TOKEN.keySetting].orEmpty(),
                        userId = settings.value[SettingKey.ID.keySetting].orEmpty()
